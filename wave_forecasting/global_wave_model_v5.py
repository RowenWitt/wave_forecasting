...
def diagnose_mesh_memory(mesh_path: str):
    """Diagnose memory requirements for mesh"""
    import pickle
    ...
    n_nodes = mesh_data['vertices'].shape[0]
    
    print(f"\n🔍 Mesh Diagnostics:")
    print(f"   Nodes: {n_nodes:,}")
    print(f"   Total edges: {n_edges:,}")
    print(f"   Avg edges per node: {n_edges / n_nodes:.1f}")
    
    # Estimate memory for message passing
    hidden_dim = 256
    edge_dim = 3
    bytes_per_float = 4
    
    # Memory for edge features during message passing
    edge_feature_memory = n_edges * (2 * hidden_dim + edge_dim) * bytes_per_float / 1024**3
    print(f"\n   Estimated memory per layer:")
    print(f"   Edge features: {edge_feature_memory:.2f} GB")
    
    # Memory for node features
    node_memory = n_nodes * hidden_dim * bytes_per_float / 1024**3
    print(f"   Node features: {node_memory:.3f} GB")
    
    # Total for all layers
    n_layers = 6
    total_memory = n_layers * (edge_feature_memory + node_memory)
    print(f"\n   Total for {n_layers} layers: {total_memory:.2f} GB")
...
# ============================================================================
class UltraMemoryEfficientGlobalWaveGNN(nn.Module):
    """V4 model with proper chunked processing"""
    
    def __init__(self, config: GlobalWaveConfigV4):
        super().__init__()
        self.config = config
        self.chunk_size = 10000  # Fixed chunk size
        
        # Rest of init...
        self.input_dim = len(config.input_features)
        self.hidden_dim = min(128, config.hidden_dim)
        ...
        self.n_mp_layers = min(3, config.n_message_layers)
        self.message_layers = nn.ModuleList([
            MemoryEfficientEdgeLayer(
                self.hidden_dim, 
                3,
                self.hidden_dim // 2,
                config.edge_dropout
        ...
        self.temporal_agg = nn.Linear(self.hidden_dim * config.sequence_length, self.latent_dim)
        self.output_head = nn.Linear(self.latent_dim, 4)
    
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor,
                edge_slices: Optional[Dict[str, slice]] = None) -> torch.Tensor:
        """Forward pass - for large graphs, only process first chunk"""
        batch_size, seq_len, num_nodes, _ = x.shape
        
        # For very large graphs, just process first chunk
        if num_nodes > 20000:
            print(f"⚠️  Processing subset of {self.chunk_size} nodes out of {num_nodes}")
            
            # Take first chunk of nodes
            x_chunk = x[:, :, :self.chunk_size, :]
            
            # Filter edges for this chunk
            mask = (edge_index[0] < self.chunk_size) & (edge_index[1] < self.chunk_size)
            chunk_edges = edge_index[:, mask]
            chunk_attr = edge_attr[mask]
            
            # Process chunk
            output = self._forward_chunk(x_chunk, chunk_edges, chunk_attr)
            
            # IMPORTANT: Return output with same size as chunk, not full size
            return output  # This will be [batch, chunk_size, 4]
        else:
            # Process full graph for smaller cases
            return self._forward_chunk(x, edge_index, edge_attr)
    
    def _forward_chunk(self, x, edge_index, edge_attr):
        """Process a single chunk or full graph"""
        batch_size, seq_len, num_nodes, _ = x.shape
        ...
        for t in range(seq_len):
            ...
            for b in range(batch_size):
                ...
                # Message passing with available edges
                if edge_index.shape[1] > 0:
                    for layer in self.message_layers:
                        h = layer(h, edge_index, edge_attr)
                
                batch_hidden.append(h)
            
            h_t = torch.stack(batch_hidden, dim=0)
            all_hidden.append(h_t)
        
        # Temporal aggregation
        hidden_cat = torch.cat(all_hidden, dim=-1)
        output_hidden = self.temporal_agg(hidden_cat)
        
        # Output
        output = self.output_head(output_hidden)
        output[:, :, 1:3] = F.normalize(output[:, :, 1:3], p=2, dim=-1)
        
        return output


class MemoryEfficientEdgeLayer(nn.Module):
    """Simplified edge layer with minimal memory usage"""
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor) -> torch.Tensor:
        ...
        if edge_index.shape[1] > 200000:
            # Random sampling
            n_sample = 200000
            perm = torch.randperm(edge_index.shape[1])[:n_sample]
            row = row[perm]
            col = col[perm]
        ...
        aggr = aggr.index_add(0, col, messages)
        
        # Update
        node_features = torch.cat([x, aggr], dim=-1)
        update = self.node_update(node_features)
        
        return self.layer_norm(x + update)
...
class GlobalWaveTrainerV4:
    ...
    def train_epoch(self, epoch: int) -> float:
        """Train one epoch with support for chunked predictions"""
        self.model.train()
        total_loss = 0.0
        
        # Determine chunk size from model if it has one
        model_chunk_size = getattr(self.model, 'chunk_size', None)
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            try:
                ...
                inputs = (inputs - self._feature_center) / self._feature_scale
                
                # Normalize targets
                targets_flat = targets.view(-1, 3).cpu().numpy()
                targets_norm = self.train_dataset.target_normalizer.transform_targets(targets_flat)
                ...
                # If model returns chunked predictions, chunk the targets to match
                pred_nodes = predictions.shape[1]
                if pred_nodes < num_nodes:
                    print(f"   Chunking targets to match predictions: {pred_nodes} nodes")
                    targets = targets[:, :pred_nodes, :]
    ...
    def validate(self) -> Dict[str, float]:
        """Validate model with support for chunked predictions"""
        self.model.eval()
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                # Check if model will chunk
                will_chunk = num_nodes > 20000
                chunk_size = 10000 if will_chunk else num_nodes
                
                # Normalize inputs
                if not hasattr(self, '_val_feature_scale'):
                    self._val_feature_scale = torch.tensor(
                        self.val_dataset.feature_normalizer.scale_, 
                        device=self.device, dtype=torch.float32
                    ).view(1, 1, 1, -1)
                    self._val_feature_center = torch.tensor(
                        self.val_dataset.feature_normalizer.center_,
                        device=self.device, dtype=torch.float32
                    ).view(1, 1, 1, -1)
                
                inputs = (inputs - self._val_feature_center) / self._val_feature_scale
                ...
                # If chunking, also chunk the targets
                if will_chunk:
                    targets = targets[:, :chunk_size, :]
...
def main():
    """Main training function with ultra memory optimization"""
    
    # First, diagnose the mesh
    mesh_path = "cache/mesh_v4.pkl"
    if Path(mesh_path).exists():
        n_nodes, n_edges = diagnose_mesh_memory(mesh_path)
        
        if n_edges > 1_000_000:
            print(f"\n⚠️  WARNING: {n_edges:,} edges will require significant memory!")
            print(f"   Consider reducing edge connectivity or using the ultra-efficient model")
    
    # Ultra-small configuration
    config = GlobalWaveConfigV4(
        experiment_name="global_wave_v4_ultra_efficient",
        num_epochs=200,
        
        # Minimal settings
        batch_size=1,
        learning_rate=1e-4,
        samples_per_epoch=100,  # Very few
        val_samples=20,
        
        # Reduced model
        hidden_dim=128,  # Much smaller
        latent_dim=32,
        n_message_layers=3,  # Fewer layers
        
        # Other settings
        gradient_clip=5.0,  # More aggressive clipping
    )
    ...
    # Use ultra-efficient model
    USE_ULTRA_EFFICIENT = True
    ...
    if USE_ULTRA_EFFICIENT:
        original_setup = trainer.setup_model
        
        def setup_ultra_model(mesh_path):
            # Load mesh data
            with open(mesh_path, 'rb') as f:
                mesh_data = pickle.load(f)
            
            trainer.edge_index = mesh_data['edge_index'].to(trainer.device)
            trainer.edge_attr = mesh_data['edge_attr'].to(trainer.device)
            trainer.edge_slices = mesh_data.get('edge_slices', None)
            
            # Create ultra-efficient model
            trainer.model = UltraMemoryEfficientGlobalWaveGNN(config).to(trainer.device)
            
            # Rest of setup
            trainer.criterion = WaveLoss(config)
            ...
            print(f"   ✅ Ultra-efficient model parameters: {n_params:,}")
        
        trainer.setup_model = setup_ultra_model
