#!/usr/bin/env python3
"""
Global Wave Model Evaluation Script - V3 Hybrid Sampling Version
...
# Import from the V3 hybrid sampling model
...
class GlobalWaveEvaluatorV3:
    """Main evaluation class for global wave model V3 with hybrid sampling"""
    ...
    def __init__(self, checkpoint_path: str, data_paths: List[str], output_dir: str = "evaluation_results"):
        ...
        print(f"📊 Model type: Multiscale with {len(self.edge_slices)} edge types")
    
    def load_model(self):
        """Load multiscale model from checkpoint"""
        print("   🔗 Loading multiscale model with hybrid sampling support")
    ...
    def prepare_validation_data(self, max_samples: Optional[int] = None):
        ...
        dataset = BathymetryEnabledHybridSamplingDataset(
    ...
    def plot_regional_performance(self, regional_results, save_path: Optional[str] = None):
        ...
        plt.suptitle('Regional Model Performance - V3 Hybrid Sampling', fontsize=16, y=0.98)
    ...
    def save_metrics_report(self, global_metrics, regional_results):
        ...
        report = {
            ...
            'data_paths': [str(p) for p in self.data_paths],
            'model_type': 'MultiscaleGlobalWaveGNN_V3_HybridSampling',
        ...
        with open(self.output_dir / 'evaluation_summary.txt', 'w') as f:
            f.write("GLOBAL WAVE MODEL V3 EVALUATION REPORT\n")
            f.write("=" * 50 + "\n\n")
    ...
    def run_complete_evaluation(self, max_samples: Optional[int] = None):
        ...
        print("\n🚀 Starting Global Wave Model V3 Evaluation")
        print("=" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate Global Wave Model V3')
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='Path to model checkpoint')
    parser.add_argument('--data', type=str, nargs='+', required=True,
                       help='Path(s) to validation data (NetCDF files)')
    parser.add_argument('--output_dir', type=str, default='evaluation_results_v3',
